...
contract TmpAddress is Ownable {
    constructor() {}
 ...
 */
contract LKKToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        setWhiteLists(address(this));
        setWhiteLists(_msgSender());
    }
    ...
    function setTokensSellThreshold(uint256 v) public onlyOwner {
        numTokensSellToAddToLiquidity = v;
    }

    function setWhiteLists(address _target) public onlyOwner {
        require(!whitelist[_target], "already whitelisted");

        whitelist[_target] = true;
        emit UpdateWhiteList(_target, 1);
    }

    function removeWhiteLists(address _target) public onlyOwner {
        require(whitelist[_target], "not whitelist");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 newBalance = IERC20(usdtAddress).balanceOf(address(tmpAddress));
        tmpAddress.transfer(usdtAddress, newBalance);
        newBalance = IERC20(usdtAddress).balanceOf(address(this));
    ...
    function _transfer(
        ...
        if (whitelist[sender] || whitelist[recipient]) {
            ...
            transferFeeRate = recipient == uniswapV2Pair ? sellFeeRate : (sender == uniswapV2Pair ? buyFeeRate : 0);
